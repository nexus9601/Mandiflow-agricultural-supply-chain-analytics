...
def render(df: pd.DataFrame, prices_df: pd.DataFrame, filters: dict) -> None:
    ...
    kpi_data = [
        (c1, "Avg Modal Price",   metrics.fmt_inr(kpis["avg_modal_price"]),      "default"),
        (c2, "Avg MSP",           metrics.fmt_inr(kpis["avg_msp"]),              "default"),
        (c3, "Avg MSP Gap",       metrics.fmt_inr(kpis["avg_msp_gap"]),
         "danger" if (kpis["avg_msp_gap"] or 0) < 0 else "default"),
        (c4, "Price Crash Count", metrics.fmt_int(kpis["price_crash_count"]),    "danger"),
        (c5, "Price Crash Rate",  metrics.fmt_pct(kpis["price_crash_rate"]),     "amber"),
        (c6, "Avg Price Spread",  metrics.fmt_inr(kpis["avg_price_spread"]),     "default"),
    ]
