...
def render(df: pd.DataFrame, transport_df: pd.DataFrame) -> None:
    ...
    with c1:
        st.markdown(styles.kpi_card("Total Checks", str(len(checks))), unsafe_allow_html=True)
    with c2:
        st.markdown(styles.kpi_card("Passed (Good)", str(good_count), "default"), unsafe_allow_html=True)
    with c3:
        st.markdown(styles.kpi_card("Warnings", str(warn_count), "amber"), unsafe_allow_html=True)
    with c4:
        st.markdown(styles.kpi_card("Critical Issues", str(crit_count), "danger"), unsafe_allow_html=True)
