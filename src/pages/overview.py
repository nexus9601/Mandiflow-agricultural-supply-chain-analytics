...
def render(df: pd.DataFrame, transport_df: pd.DataFrame, prices_df: pd.DataFrame) -> None:
    st.markdown(styles.page_header(
        ...
        "Unified cross-domain intelligence integrating mandi inflows, wholesale price discovery, MSP parity, and transit logistics.",
    ...
    kpi_data = [
        ...
        (c3, "Average MSP",       metrics.fmt_inr(kpis["avg_msp"]),           "amber",   "Support Floor",       "🛡️"),
        (c4, "Price Crash Count", metrics.fmt_int(kpis["price_crash_count"]), "danger",  "Below MSP Floor",     "⚠️"),
        (c5, "Avg Transit Time",  metrics.fmt_hrs(kpis["avg_transit_hours"]), "sky",     "Dispatch to Mandi",   "⏱️"),
    ...
    # ── Chart Row 1 ──────────────────────────────────────────────────────────
    st.markdown(styles.section_header("Arrival Dynamics & Commodity Distribution", "INFLOW ANALYTICS"), unsafe_allow_html=True)
...
<div class="insight-card">
  <h4>💡 Strategic Supply Chain Takeaways</h4>
  <div class="insight-grid">
    <div class="insight-item">
      <div class="insight-label">Dominant Inflow Crop</div>
      <div style="font-weight: 700; font-size: 0.95rem; color: #059669; margin-top: 2px;">{insights.get('top_crop', '—')}</div>
      <div style="font-size: 0.72rem; color: #64748b; margin-top: 2px;">Highest total arrivals in selected period</div>
    </div>
    <div class="insight-item">
      <div class="insight-label">Primary Trading Hub</div>
      <div style="font-weight: 700; font-size: 0.95rem; color: #0f172a; margin-top: 2px;">{insights.get('top_mandi', '—')}</div>
      <div style="font-size: 0.72rem; color: #64748b; margin-top: 2px;">Lead mandi by aggregated receipt volume</div>
    </div>
    <div class="insight-item">
      <div class="insight-label">Severe MSP Deficit Crop</div>
      <div style="font-weight: 700; font-size: 0.95rem; color: #dc2626; margin-top: 2px;">{insights.get('worst_msp_crop', '—')}</div>
      <div style="font-size: 0.72rem; color: #64748b; margin-top: 2px;">Trading widest below guaranteed price floor</div>
    </div>
    <div class="insight-item">
      <div class="insight-label">Logistics Bottleneck</div>
      <div style="font-weight: 700; font-size: 0.95rem; color: #d97706; margin-top: 2px;">{insights.get('worst_transit_warehouse', '—')}</div>
      <div style="font-size: 0.72rem; color: #64748b; margin-top: 2px;">Warehouse registering longest average transit times</div>
    </div>
  </div>
</div>
""", unsafe_allow_html=True)
