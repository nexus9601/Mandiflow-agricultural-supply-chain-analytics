...
def render(df: pd.DataFrame, transport_df: pd.DataFrame, prices_df: pd.DataFrame) -> None:
    st.markdown(styles.page_header(
        "MandiFlow Intelligence Overview",
    ...
    kpi_data = [
        (c1, "Total Arrivals",      metrics.fmt_qty(kpis["total_arrivals"]),    "emerald", "🌾", "Aggregated Volume", "up"),
        (c2, "Avg Modal Price",     metrics.fmt_inr(kpis["avg_modal_price"]),   "emerald", "💰", "Market Average",   "up"),
        (c3, "Average MSP",         metrics.fmt_inr(kpis["avg_msp"]),           "amber",   "⚖️", "Support Floor",    ""),
        (c4, "Price Crash Alerts",  metrics.fmt_int(kpis["price_crash_count"]), "danger",  "🚨", "Below MSP",        "down"),
        (c5, "Avg Transit Time",    metrics.fmt_hrs(kpis["avg_transit_hours"]), "sky",     "⏱️", "Logistics Time",   ""),
        (c6, "Long Transit Rate",   metrics.fmt_pct(kpis["long_transit_rate"]), "amber",   "🚛", "> 24h Threshold",  ""),
    ]
    for col, label, val, variant, icon, delta, trend in kpi_data:
        with col:
            st.markdown(styles.kpi_card(label, val, variant=variant, delta=delta, icon=icon, trend=trend), unsafe_allow_html=True)

    # ── Chart Row 1 ──────────────────────────────────────────────────────────
    st.markdown(styles.section_header("Arrival Dynamics & Commodity Distribution", badge="REAL-TIME TRENDS"), unsafe_allow_html=True)
    ...
    # ── Chart Row 2 ──────────────────────────────────────────────────────────
    st.markdown(styles.section_header("Mandi Performance & Price vs MSP Parity", badge="MARKET BENCHMARK"), unsafe_allow_html=True)
...
<div class="insight-card">
  <h4>🌾 Executive Supply Chain Signals</h4>
  <div class="insight-grid">
    <div class="insight-item">
      <div style="font-size: 1.2rem;">🌾</div>
      <div>
        <div class="insight-label">Highest-Arrival Crop</div>
        <div style="font-weight: 700; font-size: 0.92rem; color: #0f172a; margin-top: 2px;">{insights.get('top_crop', '—')}</div>
      </div>
    </div>
    <div class="insight-item">
      <div style="font-size: 1.2rem;">🏢</div>
      <div>
        ...
        <div style="font-weight: 700; font-size: 0.92rem; color: #0f172a; margin-top: 2px;">{insights.get('top_mandi', '—')}</div>
      </div>
    </div>
    <div class="insight-item">
      ...
      <div>
        ...
        <div style="font-weight: 700; font-size: 0.92rem; color: #dc2626; margin-top: 2px;">{insights.get('worst_msp_crop', '—')}</div>
      </div>
    </div>
    <div class="insight-item">
      <div style="font-size: 1.2rem;">🚚</div>
      <div>
        ...
        <div style="font-weight: 700; font-size: 0.92rem; color: #0f172a; margin-top: 2px;">{insights.get('worst_transit_warehouse', '—')}</div>
      </div>
    </div>
  </div>
</div>
